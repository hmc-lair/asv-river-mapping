...
import utm
import time
import digi.xbee.models as xbee_models
...
class ASV_graphics:
    ...
    def on_quit(self):
        '''Quitting out of the gui window'''
    ...
    def update(self):
        if self.controller.mode == "SIM MODE":
            # call the sim ASV loop
            

            # Get ASV coordinates
            self.controller.robot_state = self.controller.ASV_sim.state_est
    ...
    def go_to_location(self, col, row):
        ...
        if self.controller.mode == "HARDWARE MODE":
            ...
            origin_xbee_message = xbee_models.message.XBeeMessage(command_msg.encode(), None, None)
            wp_xbee_message = xbee_models.message.XBeeMessage(way_point_msg.encode(), None, None)
            self.controller.ASV_sim.xbee_callback(origin_xbee_message)
            self.controller.ASV_sim.xbee_callback(wp_xbee_message)
